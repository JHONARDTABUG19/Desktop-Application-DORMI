...
def main(login_root=None):
    ...
    # ── STUDENTS PAGE (refined) ─────────────────────────────
    def build_students_page(page):

        # ── Add / Edit Student window ────────────────────────
        def add_student_window(prefill=None, edit_item=None):
            """
            prefill tuple: (student_no, first_name, surname, mi, program, status, contact)
            The table stores:  (student_no, display_name, program, room, status, contact)
            Room is intentionally excluded from this form — use Assign Room instead.
            """
            win = Toplevel()
            ...
            midFrame.columnconfigure(0, weight=3)
            midFrame.columnconfigure(1, weight=3)
            midFrame.columnconfigure(2, weight=1)

            # ── helper: bordered entry ───────────────────────
            def make_entry(parent, row, col, colspan=1, padx=(0, 0), width=None):
                border = tk.Frame(parent, bg=WHITE,
                                  highlightbackground=BORDER, highlightthickness=1)
                border.grid(row=row, column=col, columnspan=colspan,
                            sticky="we", padx=padx, pady=(0, 10))
                kw = dict(bg=WHITE, fg=BLACK, font=UNIFORM_FONT,
            ...
            # Row 0 – Student No. (full width)
            tk.Label(midFrame, text="Student No.", bg=WHITE, fg=FG_DARK,
                     font=UNIFORM_FONT).grid(row=0, column=0, columnspan=3,
                                             sticky="w", pady=(0, 2))
            e_no = make_entry(midFrame, row=1, col=0, colspan=3)

            # Row 2 – Name labels
            tk.Label(midFrame, text="Surname", bg=WHITE, fg=FG_DARK,
                     font=UNIFORM_FONT).grid(row=2, column=0, sticky="w",
                                             padx=(0, 6), pady=(0, 2))
            tk.Label(midFrame, text="First Name", bg=WHITE, fg=FG_DARK,
                     font=UNIFORM_FONT).grid(row=2, column=1, sticky="w",
                                             padx=6, pady=(0, 2))
            tk.Label(midFrame, text="M.I.", bg=WHITE, fg=FG_DARK,
                     font=UNIFORM_FONT).grid(row=2, column=2, sticky="w",
                                             padx=(6, 0), pady=(0, 2))

            # Row 3 – Name entries
            e_surname    = make_entry(midFrame, row=3, col=0, padx=(0, 6))
            e_firstname  = make_entry(midFrame, row=3, col=1, padx=6)
            e_mi         = make_entry(midFrame, row=3, col=2, padx=(6, 0), width=4)

            # Row 4 – Program / Status labels
            tk.Label(midFrame, text="Program", bg=WHITE, fg=FG_DARK,
                     font=UNIFORM_FONT).grid(row=4, column=0, sticky="w",
                                             padx=(0, 6), pady=(0, 2))
            tk.Label(midFrame, text="Status", bg=WHITE, fg=FG_DARK,
                     font=UNIFORM_FONT).grid(row=4, column=1, columnspan=2,
                                             sticky="w", padx=6, pady=(0, 2))

            # Row 5 – Program entry / Status combobox
            e_program = make_entry(midFrame, row=5, col=0, padx=(0, 6))

            statusVar = tk.StringVar()
            statusDrop = ttk.Combobox(midFrame, textvariable=statusVar,
                                      values=["Active", "Inactive", "On Leave"],
                                      state="readonly", font=UNIFORM_FONT)
            statusDrop.grid(row=5, column=1, columnspan=2, sticky="we",
                            padx=6, pady=(0, 10))
            statusDrop.current(0)

            # Row 6 – Contact label
            tk.Label(midFrame, text="Contact", bg=WHITE, fg=FG_DARK,
                     font=UNIFORM_FONT).grid(row=6, column=0, columnspan=3,
                                             sticky="w", pady=(0, 2))
            e_contact = make_entry(midFrame, row=7, col=0, colspan=3)

            # ── Pre-fill when editing ────────────────────────
            # prefill = (student_no, display_name, program, room, status, contact)
            # We reconstruct name parts best-effort: "Surname, Firstname MI"
            if prefill:
                e_no.insert(0, prefill[0])
                # Try to split stored display_name back into parts
                full = prefill[1]  # e.g. "dela Cruz, Juan D."
                if "," in full:
                    sn, rest = full.split(",", 1)
                    rest = rest.strip()
                    parts = rest.split()
                    fn = " ".join(parts[:-1]) if len(parts) > 1 else rest
                    mi = parts[-1].rstrip(".") if len(parts) > 1 else ""
                else:
                    sn, fn, mi = full, "", ""
                e_surname.insert(0, sn.strip())
                e_firstname.insert(0, fn.strip())
                e_mi.insert(0, mi.strip())
                e_program.insert(0, prefill[2])
                statusVar.set(prefill[4])
                e_contact.insert(0, prefill[5])

            # ── Error label ──────────────────────────────────
            err_label = tk.Label(win, text="", fg="#c0392b",
                                 bg=content_color, font=UNIFORM_FONT)
            err_label.pack()
            ...
            def save():
                ...
                firstname = e_firstname.get().strip()
                mi        = e_mi.get().strip()
                ...
                if not no:
                    err_label.config(text="Student No. is required.")
                    return
                if not surname or not firstname:
                    err_label.config(text="Surname and First Name are required.")
                    return

                # Build display name: "Surname, Firstname M."
                mi_part = f" {mi.upper()}." if mi else ""
                display_name = f"{surname}, {firstname}{mi_part}"

                # Keep existing room if editing; blank for new students
                existing_room = prefill[3] if prefill else ""

                values = (no, display_name, program, existing_room, status, contact)
        ...
        def assign_room():
            ...
            # Pre-select current room if already assigned
            if values[3] and values[3] in ["101", "102", "103", "104", "105", "106"]:
                roomDrop.set(values[3])
            else:
                roomDrop.current(0)

            # ── Confirm saves the new room into the table ────
            def confirm():
                new_room = roomVar.get()
                # values: (student_no, name, program, room, status, contact)
                updated = (values[0], values[1], values[2],
                           new_room, values[4], values[5])
                tree.item(selected[0], values=updated)
                win.destroy()
            ...
            tk.Button(bottomFrame, text="Confirm", font=UNIFORM_FONT,
                      bg=BLACK, fg=WHITE, relief="flat", padx=12, pady=5,
                      cursor="hand2", command=confirm).pack(side="right", padx=(5, 0))
            tk.Button(bottomFrame, text="Cancel", font=UNIFORM_FONT,
                      fg="#c0392b", relief="flat", padx=12, pady=5,
        ...
        # ── Treeview ──────────────────────────────────────────
        # Columns: student_no | name (Surname, Firstname M.) | program | room | status | contact
