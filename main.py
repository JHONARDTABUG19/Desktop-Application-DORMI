...
def main():
    ...
    def create_table_cleaning_staff():
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # ROOMS PAGE  — fixed version
    # Key change: tree, room_count_label, and update_room_count are all created
    # BEFORE the action functions that reference them, so closures resolve correctly.
    # The "+ Add room" button is created early but its command is wired AFTER
    # add_room_window is defined.
    # ──────────────────────────────────────────────────────────────────────────
    def build_rooms_page(page):
        ...
        # ── Top bar (button command wired later) ─────────────
        topbar = tk.Frame(page, bg=content_color)
        topbar.pack(fill="x", padx=28, pady=(20, 12))
        ...
        # ── count label and update_room_count defined here ───
        # so add_room_window / delete_room / etc. can reference them
        room_count_label = tk.Label(action_bar, text="6 rooms total", bg=WHITE, fg=FG_MUTED,
                                    font=UNIFORM_FONT)
        ...
        # ── Action functions (tree + room_count_label exist now) ──
        def add_room_window(prefill=None, edit_item=None):
            ...
            def save_room():
                ...
                if edit_item:
                    orig = tree.item(edit_item, "values")
                    occupants    = orig[3]
                    last_cleaned = orig[5]
                    tree.item(edit_item, values=(room_no, r_type, cap, occupants, status, last_cleaned))
                else:
                    tree.insert("", "end", values=(room_no, r_type, cap, "0", status, "—"))
                    update_room_count()
        ...
        # ── Wire up buttons now that all functions are defined ─
