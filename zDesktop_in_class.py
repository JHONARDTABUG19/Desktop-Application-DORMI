...
class Database:
    ...
    def migrate_cleaning_schedule_table(self):
        """Ensure building column exists on legacy DBs."""
        with sqlite3.connect(DB_NAME) as con:
            try:
                con.execute("ALTER TABLE cleaning_schedule ADD COLUMN building VARCHAR(255) DEFAULT ''")
                con.commit()
            except Exception:
                pass
...
class main(tk.Tk):
    ...
    def build_dashboard_page(self, page):
        ...
        self.troNum = make_card(cards_frame, "#C48B2A", "🛏️", "troNum", "Rooms occupied")
